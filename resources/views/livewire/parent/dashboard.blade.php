<div>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            ...
            {{-- DEBUG: Show bookedCarebuddyIds and carebuddies IDs --}}
            <pre>bookedCarebuddyIds: {{ json_encode($bookedCarebuddyIds) }}</pre>
            <pre>carebuddies: {{ json_encode($carebuddies->pluck('id')) }}</pre>
                    ...
                    <div
                        ...
                        <div class="p-5">
                            <div class="flex items-center mb-4">
                                <div
                                    class="flex-shrink-0 h-12 w-12 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-500">
                                    @if($carebuddy->profile_photo)
                                        <img src="{{ $carebuddy->profile_photo }}" alt="{{ $carebuddy->user->name }}"
                            ...
                            <div class="mt-2 space-y-2">
                                <div class="flex items-center text-sm text-gray-500">
                                    ...
                                    Service radius:
                                    {{ $carebuddy->service_radius ? $carebuddy->service_radius . ' km' : 'N/A' }}
                                </div>
                                <div class="flex items-center text-sm text-gray-500">
                                    <svg class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400"
                                        xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <div class="mt-4 flex gap-2">
                                <a href="{{ route('parent.carebuddy.profile', $carebuddy->id) }}"
                                    class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                    View Profile
                                </a>
                                @if(in_array($carebuddy->id, $bookedCarebuddyIds))
                                    <button
                                        class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm bg-gray-400 text-white opacity-70 cursor-not-allowed"
                                        disabled>
                                        Booked
                                    </button>
                                @else
