...
<head>
    ...
    <title>{{ config('app.name', 'KidCare') }} - PlayPal Dashboard</title>
    ...
    <div class="min-h-screen bg-gray-100">
        ...
        <div class="flex">
            <div class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform sm:transform-none sm:opacity-100 transition duration-200"
                id="sidebar">
                <div class="flex items-center justify-between h-16 px-6 bg-[#ffb347]">
                ...
                <nav class="px-4 py-6">
                    <a href="{{ route('playpal.dashboard') }}"
                        class="flex items-center p-2 mb-2 text-gray-800 hover:bg-yellow-50 rounded-lg {{ request()->routeIs('playpal.dashboard') ? 'bg-yellow-100' : '' }}">
                    ...
                    <a href="{{ route('playpal.bookings') }}"
                        class="flex items-center p-2 mb-2 text-gray-800 hover:bg-yellow-50 rounded-lg {{ request()->routeIs('playpal.bookings') ? 'bg-yellow-100' : '' }}">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('playpal.profile.show') }}"
                        class="flex items-center p-2 mb-2 text-gray-800 hover:bg-yellow-50 rounded-lg {{ request()->routeIs('playpal.profile.show') ? 'bg-yellow-100' : '' }}">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="mt-8">
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit"
                                class="flex items-center p-2 w-full text-gray-800 hover:bg-yellow-50 rounded-lg">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
